...
import { isChatEnabled, upsertChat, insertFeedback } from "../db/index.js";
...
export async function startBot(): Promise<void> {
  ...
  bot.command("start", async (ctx) => {
    await ctx.reply(
      "RM Bot\n\n/task — ответом на сообщение менеджера\n/feedback текст — заявка на доработку (сохраняется, Cursor — если настроен)\n/web — ссылка админки (Ivan)\n/ping — проверка",
    );
  });
  ...
  bot.command("feedback", async (ctx) => {
    try {
      ...
      await ctx.reply(
        `Заявка #${id} сохранена.\nCursor-агент запустится только если на ноутбуке задан CURSOR_REPO_URL.`,
      );
      void spawnFeedbackAgent(id, body);
    } catch (e) {
